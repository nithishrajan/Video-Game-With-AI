...
class Move {
    ...
    boolean isExtend() {
        int moveSize = abs(_toIndex - _fromIndex);
        if (moveSize == 1 || moveSize == 11) {
            return true;
        } else if (moveSize == 10 || moveSize == 12) {
            return true;
        }
        return false;
    }
    ...
    boolean isJump() {
        int moveSize = abs(_toIndex - _fromIndex);
        int [] jList = new int[]{2, 9, 13};
        int sLimit = 10 * 2;
        int tLimit = 8 * 3;
        for (int i : jList) {
            if (i == moveSize) {
                return true;
            } else if (moveSize >= sLimit && moveSize <= tLimit) {
                return true;
            }
        }
        return false;
    ...
    public String toString() {
        String moveString = String.valueOf(_col0);
        String passString = "-";
        if (isPass()) {
            return passString;
        } else {
